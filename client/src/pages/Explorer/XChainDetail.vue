...
export default {
  ...
  methods: {
    async getTransactions() {
      try {
        const { data } = await this.$axios.get(`/api/tx/xtransactions?page=1`);
    ...
    async onSearch() {
      ...
      this.loading = true;
      const isAddress = this.keyword.startsWith('X-fuji');
      if (isAddress) {
        try {
        ...
        try {
          ...
          if(data.status === 200) {
            ...
            console.log(' get X-Chain transaction failed...', data.status);
          }
        } catch (error) {
            console.info(error);
        }
